...
"""Esta funcion recibe por paramenotro una lista de tuplas (objeto, probabilidad), una lista de objetos y un objeto
Lo que se hace basicamente es anadir el objeto a la solucion y quitarlo de los posibles seleccionables"""
def remove_objeto(objetos,solucion,obj):
        solucion.append(obj[0])
        objetos.remove(obj)
...
def lanza_hormiga(capacidad, objs_prob, feromonasSalida):
    ...
    #Se comprueba si cambiando un objeto de la mochila por otro de fuera mejora la solucion, si mejora se hace el cambio y se puede a buscar.
    
    while hace_cambio:
        numb = random.randrange(len(solucion))
        obj_saliente = solucion[numb]
        peso_total2 = peso_total - obj_saliente.peso
        valor_total2 = valor_total - obj_saliente.valor
        obj_entrante = selecciona_objeto(objs_prob, capacidad, peso_total2);
        
        if obj_entrante != None:
            
            if (valor_total2+obj_entrante[0].valor)>valor_total:
                peso_total = peso_total2+obj_entrante[0].peso
                valor_total = valor_total2+obj_entrante[0].valor
                solucion.remove(obj_saliente)
                objs_prob.append((obj_saliente, 0.0)) #se le pone un valor bajo de probabilidad ya que ha sido eliminado de la solucion
                add_objeto(objs_prob, solucion, obj_entrante)
            else:
